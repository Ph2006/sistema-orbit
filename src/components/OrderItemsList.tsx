...
const OrderItemsList: React.FC<OrderItemsListProps> = ({ order, onClose, onUpdateOrder }) => {
  ...
  const handleQuickConfigureGoogleDrive = () => {
    const newLink = prompt('Digite o link do Google Drive:', googleDriveLink || 'https://drive.google.com/drive/folders/');
    if (newLink !== null) {
  ...
  const calculateItemWeight = (item: OrderItem) => (item.quantity || 0) * (item.unitWeight || 0);
  const calculateTotalWeight = () => items.reduce((total, item) => total + calculateItemWeight(item), 0);
