...
const OrderModal: React.FC<OrderModalProps> = ({
  ...
  // Atualizar nome do projeto quando projectId mudar
  useEffect(() => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const orderToSave: Order = {
      ...formData,
      // Garantir que datas sejam strings ISO
...
const OrderModal: React.FC<OrderModalProps> = ({
  ...
  const [formData, setFormData] = useState<Order>({
    ...order,
    // Manter TODOS os campos originais do pedido
    orderNumber: order.orderNumber || '',
    internalOrderNumber: order.internalOrderNumber || '',
    customer: order.customer || '',
    customerName: order.customerName || '',
    ...
    // Preservar campos de auditoria e controle
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Preparar dados para salvamento mantendo todos os campos originais
    const orderToSave: Order = {
      ...formData,
      // Garantir que datas sejam strings ISO
      startDate: formData.startDate || new Date().toISOString(),
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        {/* Header Simplificado - SEM resumo */}
        <div className="flex justify-between items-center p-6 border-b border-gray-200">
          <h2 className="text-2xl font-bold text-gray-900">
            {order.id === 'new' ? 'Novo Pedido' : `Pedido #${formData.orderNumber}`}
          </h2>
