...
const itemSchema = z.object({
  ...
  // Campo legado: mantém compatibilidade com cotações antigas.
...
const calculateItemTotals = (item: Item | any, discountPercent: number = 0) => {
    ...
    const priceAfterDiscount = totalPrice * (1 - discountPercent / 100);

    const hasSeparatedTaxes = [
        item.icmsRate,
        item.pisRate,
        item.cofinsRate,
        item.irpjRate,
    ...
    if (!hasSeparatedTaxes) {
        ...
        return {
            totalPrice,
            priceAfterDiscount,
            discountAmount: originalTotalWithTax - totalWithTax,
            icmsAmount: 0,
            pisAmount: 0,
            cofinsAmount: 0,
            irpjAmount: 0,
        ...
        try {
            ...
            const itemsWithTotals = values.items.map(item => {
                ...
                return {
                    ...
                    priceAfterDiscount: totals.priceAfterDiscount,
    ...
    const handleExport = async (formatType: 'pdf' | 'excel') => {
        ...
        try {
            ...
            if (formatType === 'pdf') {
                ...
                y += 5;
                docPdf.text('PIS:', rightColX - 80, y, { align: 'right' });
                docPdf.text(publicTaxTotals.totalPis.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }), rightColX, y, { align: 'right' });
                y += 5;
                docPdf.text('COFINS:', rightColX - 80, y, { align: 'right' });
                                        ...
                                        <CardContent className="space-y-4">
                                            ...
                                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <Input type="number" step="0.01" value={currentItem.icmsRate ?? ''}
                                                        onChange={e => handleCurrentItemChange('icmsRate', e.target.value)} />
                                                </div>
                                                <div>
                                                    <FormLabel>PIS (%)</FormLabel>
                                                    <Input type="number" step="0.01" value={currentItem.pisRate ?? ''}
                                                        onChange={e => handleCurrentItemChange('pisRate', e.target.value)} />
                                                </div>
                                                <div>
                                                                    ...
                                                                    <TableCell className="text-center p-3 text-sm font-medium">
                                                                        {item.icmsRate !== undefined ? (
                                                                            <div className="space-y-1">
                                                                                <div>ICMS: {item.icmsRate ?? 0}%</div>
                                                                                <div>PIS: {item.pisRate ?? 0}%</div>
                                                                                <div>COFINS: {item.cofinsRate ?? 0}%</div>
                                                                            </div>
                                                                        ) : (
                                                                            <div>Legado: {item.taxRate ?? 0}%</div>
                                                                        )}
                                                                    </TableCell>
                                                                    <TableCell className="text-center p-3">
                                                                        {item.leadTimeDays ? (
                                        ...
                                        <Table>
                                            <TableHeader>
                                                <TableRow>
                                                    ...
                                                    <TableHead className="text-center w-[180px]">ICMS / PIS / COFINS</TableHead>
...
on, getDocs, addDoc, doc, updateDoc, deleteDoc, Timestamp, getDoc, setDoc } from "firebase/firestore";
import { db } from "@/lib/firebase";
import { PlusCircle, Search, Pencil, Trash2, CalendarIcon, X, PackagePlus, Percent, DollarSign, FileText, Check, FileDown, Copy, Package, ChevronUp, ChevronDown } from "lucide-react";
import { useAuth } from "../layout";
import { format } from "date-fns";
