...
export default function ProductsPage() {
  ...
  const [useOptimisticScenario, setUseOptimisticScenario] = useState(false);
  ...
  const calculateFeasibility = () => {
    ...
    // Calcula o lead time ajustado para cada item considerando sua quantidade
    const itemsWithAdjustedLeadTime = calculatorItems.map(item => {
      const itemLeadTime = item.stages.reduce((sum, stage) => sum + (stage.durationDays || 0), 0);
      const quantityAdjustedLeadTime = itemLeadTime * item.quantity;
      const adjustedStages = item.stages.map(stage => {
        let currentWorkload = sectorWorkload[stage.stageName] || 0;
        if (useOptimisticScenario) {
          currentWorkload = 0.2 + Math.random() * 0.1;
        }
        ...
        return {
          ...stage,
          adjustedDuration: Math.ceil((stage.durationDays || 0) * item.quantity * adjustmentFactor),
          workload: currentWorkload,
          adjustmentFactor
        };
      });
      const finalAdjustedLeadTime = adjustedStages.reduce((sum, stage) => sum + stage.adjustedDuration, 0);
      return {
        ...item,
        originalLeadTime: itemLeadTime,
        quantityAdjustedLeadTime,
        finalAdjustedLeadTime,
        adjustedStages
      };
    });

    // LÓGICA CORRIGIDA: Considera apenas o MAIOR lead time individual (produção em paralelo)
    const criticalItem = itemsWithAdjustedLeadTime.reduce((max, current) => 
      current.finalAdjustedLeadTime > max.finalAdjustedLeadTime ? current : max
    );

    // Analisa as etapas do item crítico para o relatório
    const analysis = criticalItem.adjustedStages.map((stage) => ({
      stageName: stage.stageName,
      originalDuration: stage.durationDays || 0,
      adjustedDuration: stage.adjustedDuration,
      workload: stage.workload,
      bottleneck: stage.workload >= 0.75
    }));

    // O lead time total é apenas o do item crítico (não soma de todos)
    const totalAdjustedLeadTime = criticalItem.finalAdjustedLeadTime;
    ...
    if (calculatorItems.length > 3) {
      confidence -= (calculatorItems.length - 3) * 5;
    }
    ...
    setCalculatorResults({
      ...
      confidence,
      criticalItem: {
        productCode: criticalItem.productCode,
        productDescription: criticalItem.productDescription,
        quantity: criticalItem.quantity
      }
    });
  };
                        ...
                        <CardContent className="space-y-6">
                            ...
                            <div>
                              <div className="flex items-center justify-between mb-3">
                                <h4 className="text-sm font-medium">Carga Atual dos Setores</h4>
                                <div className="flex items-center gap-2">
                                  <Button 
                                    variant={useOptimisticScenario ? "default" : "outline"}
                                    size="sm" 
                                    onClick={() => setUseOptimisticScenario(!useOptimisticScenario)}
                                    className="text-xs"
                                  >
                                    {useOptimisticScenario ? "Cenário Otimista Ativo" : "Usar Cenário Otimista"}
                                  </Button>
                                  <Button 
                                    variant="outline" 
                                    size="sm" 
                              ...
                              {useOptimisticScenario && (
                                <div className="mb-3 p-3 bg-blue-50 border border-blue-200 rounded-md">
                                  <div className="flex items-center gap-2 text-blue-800 text-sm">
                                    ...
                                    <span className="font-medium">Cenário Otimista Ativo</span>
                                  </div>
                                  <p className="text-xs text-blue-700 mt-1">
                                    Todas as cargas dos setores serão consideradas baixas (20-30%) para simular capacidade máxima disponível.
                                  </p>
                                </div>
                              )}
                              <div className="grid gap-2">
                                {manufacturingStages.map(stage => {
                                  const actualWorkload = sectorWorkload[stage] || 0;
                                  const displayWorkload = useOptimisticScenario ? (0.2 + Math.random() * 0.1) : actualWorkload;
                                  const percentage = Math.round(displayWorkload * 100);
                                  ...
                                  return (
                                    <div key={stage} className="flex items-center gap-3">
                                      ...
                                      {useOptimisticScenario && actualWorkload !== displayWorkload && (
                                        <span className="text-xs text-blue-600">
                                          (real: {Math.round(actualWorkload * 100)}%)
                                        </span>
                                      )}
                                    </div>
                                  );
