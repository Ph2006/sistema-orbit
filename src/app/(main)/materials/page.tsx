...
export default function MaterialsPage() {
    ...
    const handleExportAllRequisitionsPDF = async () => {
        ...
        toast({ title: "Gerando PDF de todas as requisições..." });
        ...
        try {
            ...
            docPdf.text(`Relatório de Requisições de Materiais`, pageWidth / 2, y + 5, { align: 'center' });
            
            y += 5;
            docPdf.setFontSize(9).setFont(undefined, 'normal');
            docPdf.text(`Gerado em ${format(new Date(), 'dd/MM/yyyy HH:mm')}`, pageWidth / 2, y + 5, { align: 'center' });
            
            y = 50;

            // Resumo geral
            docPdf.setFontSize(12).setFont(undefined, 'bold').text('Resumo Geral', 15, y); 
            y += 6;
            ...
            summaryData.push(['Total de Requisições', requisitions.length.toString()]);
            ...
            docPdf.setFontSize(12).setFont(undefined, 'bold').text('Lista de Requisições', 15, y); 
            y += 6;
            
            const requisitionsData = requisitions.map(req => {
                const orderInfo = orders.find(o => o.id === req.orderId);
                const totalItems = req.items.length;
                const customerName = req.customer?.name || orderInfo?.customerName || 'N/A';
                
                return [
                    req.requisitionNumber || req.id || '',
                    format(req.date, 'dd/MM/yyyy'),
                    req.requestedBy,
                    orderInfo?.internalOS || 'N/A',
                    customerName,
                    totalItems.toString(),
                    req.status
                ];
            ...
            autoTable(docPdf, { 
                startY: y, 
                head: [['Nº Req.', 'Data', 'Solicitante', 'OS', 'Cliente', 'Itens', 'Status']],
                body: requisitionsData,
                headStyles: { fillColor: [40, 40, 40] },
                styles: { fontSize: 8 },
                columnStyles: {
                    0: { cellWidth: 20 }, // Nº Req
                    1: { cellWidth: 20 }, // Data
                    2: { cellWidth: 25 }, // Solicitante
                    3: { cellWidth: 15 }, // OS
                    4: { cellWidth: 30 }, // Cliente
                    5: { cellWidth: 15 }, // Itens
            ...
            docPdf.text(`Relatório gerado em ${format(new Date(), 'dd/MM/yyyy HH:mm')}`, pageWidth / 2, pageHeight - 10, { align: 'center' });

            docPdf.save(`Relatorio_Requisicoes_${format(new Date(), 'ddMMyyyy')}.pdf`);
            
            toast({ title: "PDF gerado com sucesso!", description: "O relatório de requisições foi exportado para PDF." });
            
        } catch (error) {
            console.error("Error exporting all requisitions PDF:", error);
            toast({ variant: "destructive", title: "Erro ao exportar", description: "Não foi possível gerar o PDF do relatório." });
        }
    };
    ...
    const handleExportRequisitionPDF = async (requisition: Requisition) => {
        ...
        toast({ title: "Gerando PDF da Requisição..." });
        
        try {
            const companyRef = doc(db, "companies", "mecald", "settings", "company");
            const companySnap = await getDoc(companyRef);
            const companyData: CompanyData = companySnap.exists() ? companySnap.data() as CompanyData : {};
            const orderInfo = orders.find(o => o.id === requisition.orderId);
            ...
            // Título do documento
            docPdf.setFontSize(16).setFont(undefined, 'bold');
            docPdf.text(`Requisição de Materiais Nº ${requisition.requisitionNumber || requisition.id}`, pageWidth / 2, y + 5, { align: 'center' });
            
            y += 5;
            docPdf.setFontSize(9).setFont(undefined, 'normal');
            const customerName = requisition.customer?.name || orderInfo?.customerName || 'N/A';
            docPdf.text(`OS: ${orderInfo?.internalOS || 'N/A'} | Cliente: ${customerName}`, pageWidth / 2, y + 5, { align: 'center' });
            
            y = 50;

            // Informações da requisição
            docPdf.setFontSize(12).setFont(undefined, 'bold').text('Informações da Requisição', 15, y); 
            y += 6;
            ...
            autoTable(docPdf, { 
                ...
                styles: { fontSize: 9 }, 
                body: [ 
                    ['Data da Requisição:', format(requisition.date, 'dd/MM/yyyy')],
                    ['Solicitante:', requisition.requestedBy],
                    ['Departamento:', requisition.department || 'N/A'],
                    ['Status:', requisition.status],
                    ['OS Vinculada:', orderInfo?.internalOS || 'N/A'],
                    ['Observações Gerais:', requisition.generalNotes || 'N/A'],
                ], 
            });
            ...
            // Itens solicitados
            docPdf.setFontSize(12).setFont(undefined, 'bold').text('Itens Solicitados', 15, y); 
            y += 6;
            
            const itemsData = requisition.items.map(item => [
                item.code || '-',
                item.description,
                item.material || '-',
                item.dimensao || '-',
            ...
            autoTable(docPdf, { 
                startY: y, 
                head: [['Código', 'Descrição', 'Material', 'Dimensão', 'Qtd.', 'Unid.', 'Entrega', 'Status', 'Obs.']],
                body: itemsData,
                headStyles: { fillColor: [40, 40, 40] },
                styles: { fontSize: 8 },
                columnStyles: {
                    1: { cellWidth: 25 }, // Descrição
                    8: { cellWidth: 20 }   // Observações
                }
            });
            ...
            // Aprovação (se houver)
            if (requisition.approval?.approvedBy) {
                docPdf.setFontSize(12).setFont(undefined, 'bold').text('Informações de Aprovação', 15, y); 
                y += 6;
                ...
                autoTable(docPdf, { 
                    ...
                    styles: { fontSize: 9 }, 
                    body: [ 
                        ['Aprovado por:', requisition.approval.approvedBy],
                        ['Data da Aprovação:', requisition.approval.approvalDate ? format(requisition.approval.approvalDate, 'dd/MM/yyyy') : 'N/A'],
                        ['Justificativa:', requisition.approval.justification || 'N/A'],
                    ], 
                });
            ...
            const pageHeight = docPdf.internal.pageSize.height;
            docPdf.setFontSize(8).setFont(undefined, 'normal');
            docPdf.text(`Documento gerado em ${format(new Date(), 'dd/MM/yyyy HH:mm')}`, pageWidth / 2, pageHeight - 10, { align: 'center' });

            docPdf.save(`Requisicao_${requisition.requisitionNumber || requisition.id}.pdf`);
            
            toast({ title: "PDF gerado com sucesso!", description: "A requisição foi exportada para PDF." });
            
        } catch (error) {
            console.error("Error exporting requisition PDF:", error);
            toast({ variant: "destructive", title: "Erro ao exportar", description: "Não foi possível gerar o PDF da requisição." });
        }
    };
                                                    ...
                                                    <TableRow key={req.id}>
                                                        ...
                                                        <TableCell className="text-right">
                                                            ...
                                                            <TooltipProvider>
                                                                <Tooltip>
                                                                    <TooltipTrigger asChild>
                                                                        <Button variant="ghost" size="icon" onClick={() => handleOpenRequisitionForm(req)}>
                                                            ...
                                                            <TooltipProvider>
                                                                <Tooltip>
                                                                    <TooltipTrigger asChild>
                                                                        <Button variant="ghost" size="icon" className="text-destructive" onClick={() => handleDeleteRequisition(req)}>
