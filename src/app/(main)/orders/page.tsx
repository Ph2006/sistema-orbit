    ...
    const handleExportScheduleSummary = async () => {
        ...
        try {
            ...
            if (finalY + 30 < pageHeight - 20) {
                ...
                // Desenhos
                docPdf.setTextColor(...(docColor(selectedOrder.documents?.drawings || false) as [number, number, number]));
                docPdf.text(`Desenhos Técnicos: ${docStatus(selectedOrder.documents?.drawings || false)}`, 15, docY);
                
                // Plano de Inspeção
                docPdf.setTextColor(...(docColor(selectedOrder.documents?.inspectionTestPlan || false) as [number, number, number]));
                docPdf.text(`Plano de Inspeção: ${docStatus(selectedOrder.documents?.inspectionTestPlan || false)}`, 75, docY);
                
                // Plano de Pintura
                docPdf.setTextColor(...(docColor(selectedOrder.documents?.paintPlan || false) as [number, number, number]));
