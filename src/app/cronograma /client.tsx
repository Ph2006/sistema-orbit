  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 p-6">
      <div className="max-w-md w-full bg-white rounded-lg shadow-md p-8 text-center space-y-4">
        {status === 'loading' && <><Loader2 className="mx-auto h-10 w-10 animate-spin text-blue-600" /><p className="text-slate-600">Carregando cronograma...</p></>}
        {status === 'invalid' && <><FileWarning className="mx-auto h-10 w-10 text-red-500" /><h1 className="text-lg font-semibold">Link inválido</h1><p className="text-sm text-slate-500">Solicite um novo link ao fornecedor.</p></>}
        {status === 'error' && <><FileWarning className="mx-auto h-10 w-10 text-red-500" /><h1 className="text-lg font-semibold">Erro ao carregar</h1><p className="text-sm text-slate-500">Tente novamente em instantes.</p></>}
        {(status === 'ready' || status === 'downloaded') && <>
          <CheckCircle2 className="mx-auto h-10 w-10 text-green-600" />
          <h1 className="text-lg font-semibold">Cronograma do Pedido {orderLabel}</h1>
          <p className="text-sm text-slate-500">{status === 'downloaded' ? "Download iniciado. Se não abriu, use o botão abaixo." : "Preparando..."}</p>
          <Button onClick={() => download?.()} className="w-full"><Download className="mr-2 h-4 w-4" />Baixar Cronograma</Button>
        </>}
      </div>
    </div>
  );
